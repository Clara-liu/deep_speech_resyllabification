...
def get_all_speakers_mel(speakers: 'list speakers', combined_df_path: 'path to the combined data file',
                          condition: 'str resyllabified or non_resyllabified', nmel) -> 'save all speakers df':
    for s in speakers:
        match_onset(f'../pilot_2/{s}/normal_sound_files/{condition}', f'../pilot_2/{s}/{condition}_condition',
                    f'../pilot_2/{s}/normal_sound_files')
        current_df = get_mel_spec(f'../pilot_2/{s}/{condition}_condition', s, nmel)
...
def main(speakers: 'str which speakers',
    ...
    if speech_rate_condition == 'normal':
        get_all_speakers_mel(speakers, path_to_combined, resyllabification_condition, nmel)
    else:
